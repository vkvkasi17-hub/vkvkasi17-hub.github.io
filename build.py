...
# --- REFINED TYPOGRAPHY EXPERIENCE LOOP WITH UNIFIED DOTTED SEPARATORS ---
experience_html = '<div class="experience-container" style="display: flex; flex-direction: column; gap: 30px;">'
for exp in portfolio_data['experience']:
    # Clean bullet formatting with reduced text size and dark color
    bullets = "".join([f"<li style='margin-bottom: 10px; position: relative; padding-left: 22px; color: #1f2937; font-size: 0.95rem; line-height: 1.6;'><span style='position: absolute; left: 0; color: var(--accent);'>•</span>{b}</li>" for b in exp['bullets']])
    
    company_lower = exp["company"].lower()
    brand_graphic_html = ""
    
    # Disney Experience Layout
    if "disney" in company_lower:
        ...
        <div style="display: flex; flex: 1; justify-content: center; align-items: center; width: 100%; min-height: 180px; z-index: 1;">
            <img src="disney-logo.jpeg" 
                 style="width: 240px; height: auto; pointer-events: none; mix-blend-mode: multiply;" />
        </div>
        <div style="margin-top: auto; padding-top: 12px; border-top: 1px dashed var(--border);">
            <div style="font-size: 0.75rem; text-transform: uppercase; letter-spacing: 0.8px; color: #4b5563; font-weight: 700; margin-bottom: 8px;">Technologies Handled</div>
        ...
        # Standard Badges with Dotted Line Separator for all non-Disney companies (e.g. Cigna)
        fallback_skills = exp.get("skills", ["Python", "Backend", "SQL", "ETL"])
    ...
    experience_html += f"""
    <div class="card exp-card-grid" style="padding: 30px 35px; background: var(--card-bg); border-radius: 16px; border: 1px solid var(--border); display: grid; grid-template-columns: 30% 70%; gap: 30px; align-items: stretch; position: relative; overflow: hidden;">
        
        <!-- Left Metadata Column -->
        <div style="border-right: 1px solid var(--border); padding-right: 20px; display: flex; flex-direction: column; justify-content: space-between; position: relative; z-index: 1;">
            <div>
                <h3 style="margin: 0; font-size: 1.4rem; font-weight: 800; color: #111827; line-height: 1.2;">{exp["role"]}</h3>
                <h4 style="margin: 5px 0 10px 0; font-size: 1.05rem; color: var(--accent); font-weight: 600;">{exp["company"]}</h4>
                
                <div style="display: inline-flex; align-items: center; font-size: 0.8rem; font-weight: 600; color: #374151; background: rgba(49, 130, 206, 0.06); padding: 4px 10px; border-radius: 6px; border: 1px solid rgba(49, 130, 206, 0.15);">
        ...
        <!-- Right Column: Reduced Font Bullet Points -->
        <div style="margin: 0; padding-left: 5px; display: flex; align-items: center; position: relative; z-index: 2;">
            <ul style="margin: 0; padding: 0; list-style: none; color: #1f2937; width: 100%;">
                {bullets}
            </ul>
        </div>
